...
/**
 * Servlet implementation class FormAceptarTFGServlet
 */
@WebServlet("/FormAceptarTFGServlet")
public class FormAceptarTFGServlet extends HttpServlet {
		...
		if (tfg != null) {
			...
			if (tfg.getStatus() == 2) {
				...
				if (req.getParameter("grade") != null) {
					String m = req.getParameter("grade").replace(",", ".");
			...
			List<TFG> tfgs = client.target(URLHelper.getURL() + "/professor/"+
					tfg.getAdvisor())
